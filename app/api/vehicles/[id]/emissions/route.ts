...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (pm25 == null || pm10 == null || co2 == null) return NextResponse.json({ error: "Missing fields" }, { status: 400 })

    const emission = await prisma.emission.create({ data: { vehicleId, pm25: Number(pm25), pm10: Number(pm10), co2: Number(co2), nh3: nh3 == null ? null : Number(nh3) } })
    ...
    await prisma.vehicle.update({ where: { id: vehicleId }, data: { avgEmission: avgPm25, efficiency } })
