...
export async function POST(req: Request, context: { params: any }) {
  try {
    ...
    if (
      vehicle.ownerId &&
      String(vehicle.ownerId) !== String(user.id) &&
      user.role !== "admin"
    ) {
    ...
    try {
      const created = await (prisma as any).deviceKey.create({
        data: { vehicleId: vehicle.id, key },
      });
      ...
      console.warn(
        "Prisma deviceKey.create failed, attempting raw insert fallback:",
      ...
      if (createErr?.code === "P2031") {
        try {
          await prisma.$runCommandRaw({
            insert: "DeviceKey",
            documents: [{ vehicleId: vehicle.id, key, createdAt: new Date() }],
          });
