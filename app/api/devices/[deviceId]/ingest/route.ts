  ...
  try {
    ...
    if (!deviceId)
      return NextResponse.json(
        { error: "Missing deviceId in params" },
        { status: 400 }
      );
    ...
    if (!vehicle)
      return NextResponse.json(
        { error: "Unknown device", code: "UNKNOWN_DEVICE" },
        { status: 404 }
      );

    // authenticate device via API key (x-device-key header or body.deviceKey)
    const providedKey =
      req.headers.get("x-device-key") || (body as any).deviceKey;
    if (!providedKey)
      return NextResponse.json(
        { error: "Missing device key", code: "MISSING_KEY" },
        { status: 401 }
      );
    ...
    try {
      ...
      console.warn(
        "prisma.deviceKey.findUnique failed, attempting raw find fallback:",
        kErr
      );
      try {
    ...
    if (!keyRec || keyRec.vehicleId !== vehicle.id)
      return NextResponse.json(
        { error: "Invalid device key", code: "INVALID_KEY" },
        { status: 401 }
      );
    ...
    if (sampleId) {
      try {
        ...
        if (existing) {
          ...
          return NextResponse.json(
            { success: true, emission: existingObj },
            { status: 200 }
          );
        }
      } catch (rbErr) {
    ...
    if (pm25 == null || pm10 == null || co2 == null)
      return NextResponse.json(
        { error: "Missing required readings (pm25, pm10, co2)" },
        { status: 400 }
      );

    let emission;
    try {
      // Use relation connect (vehicle) to satisfy Prisma's create input types and keep scalar deviceId/sampleId
      ...
      console.warn(
        "Prisma emission.create failed, attempting raw Mongo insert fallback:",
      ...
      if (createErr?.code === "P2031") {
        try {
          ...
          if (!emission) {
            ...
            emission = await prisma.emission
              .findFirst({
                where: { vehicleId: vehicle.id, pm25: Number(pm25) },
          ...
          if (!emission) {
            ...
            console.warn(
              "Could not read back emission after raw insert. Continuing with constructed emission object.",
              insertRes
            );
          }
        } catch (inner) {
    ...
    try {
      ...
      console.warn(
        "Prisma vehicle.update failed, attempting raw Mongo update fallback:",
        updateErr?.code || updateErr
      );
    ...
    if (emission.pm25 >= 150 || emission.pm10 >= 250) {
      try {
        ...
        console.warn(
          "Prisma alert.create failed, attempting raw insert fallback:",
          alertErr?.code || alertErr
        );
        if (alertErr?.code === "P2031") {
          try {
            ...
            console.error("Fallback alert insert failed", inner);
          }
        }
      }
      try {
        ...
        console.warn(
          "Prisma report.create failed, attempting raw insert fallback:",
          reportErr?.code || reportErr
        );
        if (reportErr?.code === "P2031") {
          try {
            ...
            console.error("Fallback report insert failed", inner);
          }
        }
      }
