...
test.describe('Auth API (E2E)', () => {
  test('register then login should succeed and return token', async ({ request }) => {
    ...
    let regJson = null
    try { regJson = regText ? JSON.parse(regText) : null } catch (e) {
    ...
    expect(regJson?.email || regJson?.id, 'register response contains email or id').toBeTruthy()
...
test.describe('Device ingestion (E2E)', () => {
  test('ingest sensor payload and produce emission and possibly alert', async ({ request }) => {
    const unique = Date.now()
    ...
    // create user
    await request.post('/api/auth/register', { data: { name, email, password } })

    // create vehicle and link to device
    const vehicleRes = await request.post('/api/vehicles', { data: { plate: `E2E-${unique}`, model: 'E2E', year: 2025, deviceId: 'ESP32-VEH-E2E' } })
    ...
    const ingest = await request.post(`/api/devices/${payload.deviceId}/ingest`, { data: payload, headers: { 'x-device-key': deviceKey } })
    ...
    expect(list.some((e: any) => e.pm25 === 180 && e.pm10 === 300)).toBeTruthy()
    ...
    if (alerts.ok) {
      let alertsJson = null
      try {
        alertsJson = await alerts.json()
      } catch (e) {
        // not JSON (maybe no API route); skip this check
        console.warn('Alerts endpoint did not return JSON; skipping alert assertions')
      }
      if (alertsJson) {
        expect(Array.isArray(alertsJson)).toBeTruthy()
        expect(alertsJson.some((a: any) => a.message && a.message.includes('High emissions'))).toBeTruthy()
      }
    }

    // idempotency: posting same sampleId again returns 200 and not create duplicate
    const ingestAgain = await request.post(`/api/devices/${payload.deviceId}/ingest`, { data: payload, headers: { 'x-device-key': deviceKey } })
    ...
    const badIngest = await request.post(`/api/devices/${payload.deviceId}/ingest`, { data: { ...payload, sampleId: `bad-${unique}` }, headers: { 'x-device-key': 'deadbeef' } })
