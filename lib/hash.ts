...
export async function getBcrypt() {
  try {
    ...
    try {
      ...
      console.error('Failed to dynamically import bcrypt and bcryptjs. Install one of them.');
      throw new Error('Missing dependency: bcrypt or bcryptjs. Run `pnpm install bcryptjs` or ensure bcrypt is available on this platform.')
    }
  }
}
...
export async function hashPassword(password: string, rounds = 10) {
  ...
  if (b.hash.length >= 3) {
    ...
    return await new Promise<string>((resolve, reject) => {
      b.hash(password, rounds, (err: any, hashed: string) => (err ? reject(err) : resolve(hashed)))
    })
  }
  // promise-style
...
export async function comparePassword(password: string, hash: string) {
  ...
  if (b.compare.length >= 3) {
    return await new Promise<boolean>((resolve, reject) => {
      b.compare(password, hash, (err: any, same: boolean) => (err ? reject(err) : resolve(same)))
    })
  }
